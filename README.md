...
## Project Overview
The Loss Prevention Log system is an embedded IoT solution built on the M5Stack CoreS3 platform. It enables retail staff to quickly document suspicious activities or incidents in stores, using a modern touchscreen and physical button interface. The modular codebase separates the UI, SD card logging, WiFi management, and time utilities into distinct components for maintainability and scalability. All log entries are standardized, timestamped, and stored on a micro SD card. The system supports WiFi configuration, log viewing, and future extensibility (webhooks, security, etc.).
...
- **M5Stack Dual Button & Key Unit**
  ...
  - Used for menu navigation and wake-from-sleep functionality
...
### Modular Codebase Overview
- `src/ui.cpp` / `src/ui.h`: Manages all LVGL UI screens, styles, and event handling
- `src/sd_logger.cpp` / `src/sd_logger.h`: Handles SD card log storage, retrieval, and file management
- `src/wifi_handler.cpp` / `src/wifi_handler.h`: Manages WiFi connection, scanning, and credential storage
- `src/time_utils.cpp` / `src/time_utils.h`: Time, RTC, and NTP utilities
- `src/globals.h`: Shared definitions and configuration
- Main application logic (`.ino`): Initialization and module coordination

See `src/PROJECT_DOCUMENTATION.md` for a detailed component and workflow overview.
...
### Acknowledgments
- M5Stack for hardware and libraries
...
For a full technical breakdown, see [`src/PROJECT_DOCUMENTATION.md`](src/PROJECT_DOCUMENTATION.md). For a visual overview, see `project_flow.md` and `project_flowchart.md`.
